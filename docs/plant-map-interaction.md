...
- 数据来源：与既有 33 个省级要素同源（DataV GeoAtlas 行政区边界），新增原始文件 data/geo/raw/710000_self.json，
  由 scripts/build-china-geojson.py 统一生成前端边界数据；生成结果 features = 34（含 710000 台湾省，13 个岛屿环）。
- 数据库已存在 710000 台湾省（另有 810000 香港、820000 澳门），公开接口 /api/public/plant/regions/provinces 正常返回。
- 交互无需专用代码：点击/高亮/标签/省份中心/精选锚点全部按 adcode 驱动；台湾省无数据时显示「暂无公开观察」，
  点击后打开省份作品面板并给出空状态（实测 total = 0）。

### 11.2 全国精选作品瘦身（P0）

| 项 | 调整前 | 调整后 |
|---|---|---|
| 卡片尺寸 | 168 × 132 px | **128 × 102 px**（图片高度 68px） |
| 卡片信息 | 图片 + 植物名 + 花名·省·市 + 描述 | 图片 + 植物名 + **花名 · 省份** |
| 同时展示数量 | 固定 8 张 | **按地图宽度：≥1400px→8；1100~1399→6；900~1099→4；<900→0** |
| 引导线 | 1.4px / 透明度 0.55 | **1px / 透明度 0.28（hover 时 2px 高亮）** |
| 卡片与地图关系 | 可能压住中国主体 | 新增**核心保护区（横向 25%~75%）**，卡片贴左右边缘不进入中央 |
| 省份模式 | 精选层已隐藏 | 保持：进入省份后只留省份地图 + 作品侧边栏 |

布局算法（frontend/src/modules/plant-map/utils/mapLayout.ts）新增保护区分支，并补充「卡片不进入中央保护区」单测。

### 11.3 学生内容统一与旧作品收口（P0/P1）

- 学生端与公开端的旧作品页面（MyWorks / WorkSubmit / WorkList / WorkDetail）与 /student/works*、/works* 路由已删除；
  首页、菜单、登录页、待办页、管理员评论页、公开排行榜中的旧链接全部改写到植物观察链路。
- 学生首页统计改为 GET /api/student/plant/dashboard（草稿观察/待审核/已通过/被驳回）。
- 旧作品前端 API 模块（api/student/work.ts、api/public/work.ts、api/workAdapter.ts）已删除，通用上传抽到 api/upload.ts。
- 后端旧 Work 的依赖评估与分步下线路线见 docs/legacy-work-retirement.md（work 表 0 行，无历史数据需迁移）。
